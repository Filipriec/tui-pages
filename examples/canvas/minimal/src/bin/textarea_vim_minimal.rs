...
impl Default for State {
    fn default() -> Self {
        let mut textarea = canvas::TextAreaState::from_text("A simple textarea.\nType here.");
        textarea.use_wrap();
...
fn build() -> TuiApp<View, Action, State, Handler> {
    TuiPages::builder(View::Editor)
        .page_fn(page_spec)
        .handler(Handler)
        .canvas_textarea_widget_with_preset(0, canvas::BuiltinCanvasKeybindingPreset::Vim)
        .vim_defaults()
        .build()
}
...
fn render(frame: &mut Frame, state: &mut State) {
    let area = frame.area();
    let block = Block::bordered().title("textarea_vim_minimal");
